...
class Grid:
	...
	def check_bounds(self, x, y):
		return 1 <= x <= self.M+self.N-1 and 1 <= y <= self.M+self.N-1 and self.M+1 <= x+y <= 2*self.M + self.N - 1
	...
	def reflect(self):
		res = [[0 for _ in range(len(self.grid[i]))] for i in range(len(self.grid))]
	...
	def plot(self, save_path):
		...
		plt.plot([self.M, self.N+self.M-1], [self.N+self.M-1, self.M], color='r')
		plt.plot([1,1, self.M], [self.M, self.M+self.N-1, self.M+self.N-1], color='r')
		plt.plot([self.M, self.M+self.N-1, self.M+self.N-1], [1,1, self.M], color='r')
		plt.grid()
